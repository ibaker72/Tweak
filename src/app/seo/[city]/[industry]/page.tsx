...
export async function generateStaticParams() {
  return allGeoSlugs().map(({ city, state, industry }: { city: string; state: string; industry: string }) => ({
    ...
    <MarketingShell>
      ...
      <section className="mx-auto max-w-page px-5 py-12 sm:px-8">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {content.valueProps.map((v: { title: string; body: string }) => (
      ...
      <section className="mx-auto max-w-page px-5 py-12 sm:px-8">
        ...
        <div className="mt-8 divide-y divide-white/[0.06] border-y border-white/[0.06]">
          {content.faqs.map((faq: { q: string; a: string }) => (
