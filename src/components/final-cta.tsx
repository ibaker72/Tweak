...
export function FinalCTA() {
  return (
    <section className="relative overflow-hidden py-14 sm:py-32 lg:py-40">
      ...
      <div className="wrap">
        <Reveal>
          <div className="mx-auto max-w-[720px] text-center">
            ...
            <h2 className="font-display text-[clamp(28px,8vw,64px)] font-black leading-[0.92] tracking-[-0.045em] text-white sm:text-[clamp(38px,6vw,64px)]">
              Ready to build
              <br />
              <span className="gradient-text">something real?</span>
            </h2>
            <p className="mx-auto mt-4 max-w-[500px] text-[13px] leading-[1.65] text-body sm:mt-7 sm:text-[17px] sm:leading-[1.75]">
            ...
            <div className="mt-6 flex flex-col items-center justify-center gap-2.5 sm:mt-10 sm:flex-row sm:gap-3">
              <Link href="/contact" className="btn-v w-full justify-center py-2.5 text-[13px] sm:w-auto sm:px-8 sm:py-3.5 sm:text-[15px]">
            ...
            {/* Trust signals */}
            <div className="mx-auto mt-8 grid max-w-[560px] grid-cols-3 gap-1.5 sm:mt-14 sm:gap-3">
              {[
                ...
                <div
                  key={item.text}
                  className="flex items-center justify-center gap-1 rounded-full border border-white/[0.06] bg-white/[0.015] px-2 py-2 sm:gap-2 sm:px-4 sm:py-2.5"
                >
                  <item.icon size={11} className="flex-shrink-0 text-accent/60 sm:h-3 sm:w-3" />
                  <span className="text-[9px] font-medium text-white/45 sm:text-[11px]">{item.text}</span>
                </div>
              ))}
            </div>
